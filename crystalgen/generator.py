...
from crystalgen.common.data.condition_factory import ConditionLoader
from crystalgen.common.data.num_atoms_distribution import NUM_ATOMS_DISTRIBUTIONS
from crystalgen.common.data.types import TargetProperty
from crystalgen.common.utils.data_utils import lattice_matrix_to_params_torch
from crystalgen.common.utils.eval_utils import (
    CrystalGenCheckpointInfo,
    get_crystals_list,
    load_model_diffusion,
    make_structure,
    save_structures,
)
from crystalgen.common.utils.globals import DEFAULT_SAMPLING_CONFIG_PATH, get_device
...
from crystalgen.common.utils.data_classes import ProgressCallback
...
def draw_samples_from_sampler(
    ...
    hierarchical_lithiation: bool = False,
) -> list[Structure]:
    ...
    # Stage 2 of hierarchical generation: the diffusion model produces a
    # delithiated host framework; Li is then placed deterministically into its
    # interstitial voids. See crystalgen.li_placer.
    if hierarchical_lithiation:
        generated_strucs = lithiate_structures(generated_strucs)
...
def lithiate_structures(structures: list[Structure]) -> list[Structure]:
    """Insert Li into each generated host framework.

    Hosts for which no chemically sensible Li site exists (no redox-active
    transition metal, or no interstitial void that clears the steric cutoffs)
    are dropped rather than passed through unlithiated, so that downstream
    screening only ever sees candidate cathodes.
    """
    placer = PhysicsInformedLiPlacer()
    lithiated: list[Structure] = []

    for struc in structures:
        try:
            result = placer.place_lithium(struc)
        except Exception as e:
            print(f"[SKIP] Li placement failed for {struc.composition.reduced_formula}: {e}")
            continue
        if result is None:
            print(f"[SKIP] No viable Li site in {struc.composition.reduced_formula}")
            continue
        lithiated.append(result)

    print(f"Lithiation: {len(lithiated)}/{len(structures)} hosts produced a candidate cathode.")
    return lithiated
...
class CrystalGenerator:
    checkpoint_info: CrystalGenCheckpointInfo
    ...
    # Stage 2 of the hierarchical cathode pipeline: place Li into the generated
    # host frameworks. Off by default so plain CrystalGen sampling is unchanged.
    hierarchical_lithiation: bool = False
    ...
    def __post_init__(self) -> None:
        assert self.num_atoms_distribution in NUM_ATOMS_DISTRIBUTIONS, (
            ...
            "please add it to crystalgen.common.data.num_atoms_distribution.NUM_ATOMS_DISTRIBUTIONS."
        )
    ...
    def load_sampling_config(
        ...
        if not target_compositions_dict:
            ...
            # crystalgen.common.data.condition_factory.get_number_of_atoms_condition_loader
            sampling_config_overrides += [
            ...
            sampling_config_overrides += [
                "condition_loader_partial._target_=crystalgen.common.data.condition_factory.get_composition_data_loader",
    ...
    def generate(
        ...
        hierarchical_lithiation: bool | None = None,
    ) -> list[Structure]:
        ...
        generated_structures = draw_samples_from_sampler(
            ...
            hierarchical_lithiation=(
                self.hierarchical_lithiation
                if hierarchical_lithiation is None
                else hierarchical_lithiation
            ),
        )
